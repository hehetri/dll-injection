"""Decode ``mercinfo.bin`` into a readable text asset."""
from __future__ import annotations

from pathlib import Path

INPUT_FILE = Path("mercinfo.bin")
OUTPUT_DIR = Path("output")
OUTPUT_FILE = OUTPUT_DIR / "mercinfo.txt"
...
def strip_header(decoded: bytes) -> bytes:
    ...
    ``mercinfo.bin`` follows the same layout as the other map files: two 32-bit
    values precede a semicolon that starts the textual records. Removing the
    header yields clean, tab-delimited content for inspection.
    """
