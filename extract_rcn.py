"""Decode ``rcn.bin`` and emit its textual payload."""
from __future__ import annotations

from pathlib import Path

INPUT_FILE = Path("rcn.bin")
OUTPUT_DIR = Path("output")
OUTPUT_FILE = OUTPUT_DIR / "rcn.txt"
...
def strip_header(decoded: bytes) -> bytes:
    ...
    if len(decoded) >= 9 and decoded[:4] == b"\xfe\xff\xff\xff":
        # The map-style resources store two 32-bit values up front. If we
        # detect that shape, skip past it to expose the payload (usually text
