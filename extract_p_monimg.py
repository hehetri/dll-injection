"""Decode ``dungeon/p_monimg.bin`` into a readable text asset.

The file is XOR-obfuscated with ``0xFF`` and begins with an 8-byte header,
followed by ``0xFF`` padding at the end. This script removes the header and
padding after decoding, then writes the plain text to ``output/dungeon``.
"""
from __future__ import annotations

from pathlib import Path

INPUT_FILE = Path("dungeon/p_monimg.bin")
OUTPUT_DIR = Path("output/dungeon")
OUTPUT_FILE = OUTPUT_DIR / "p_monimg.txt"
...
def strip_header(decoded: bytes) -> bytes:
    """Remove the leading 8-byte header if it matches the known prefix."""
